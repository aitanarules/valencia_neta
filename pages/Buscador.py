...
# Inicializar el estado de búsqueda en la sesión
if 'buscar_nueva_ruta' not in st.session_state:
    st.session_state.buscar_nueva_ruta = True
...
def buscar_ruta():
    ...
    try:
        ...
        st.write("Nuestro módulo de localización no está disponible en estos momentos, disculpe las molestias.")
        st.warning("Puedes probar nuestra aplicación con la ubicación por defecto (Ayuntamiento de València). Para ello, deja este espacio en blanco.")
        coordenada_origen = coordenada_origen_default
    ...
    if ruta_optima:
        ...
        st.session_state.coordenada_origen = coordenada_origen
        st.session_state.graph = graph

        # Cambiar el estado para evitar nuevas búsquedas automáticas
        st.session_state.buscar_nueva_ruta = False

# Mostrar la ruta encontrada
def mostrar_ruta():
    ruta_optima = st.session_state.ruta_optima
    distancia_minima = st.session_state.distancia_minima
    contenedor_cercano = st.session_state.contenedor_cercano
    coordenada_origen = st.session_state.coordenada_origen
...
if st.session_state.buscar_nueva_ruta:
    buscar_ruta()
else:
    mostrar_ruta()

# Botón para buscar una nueva ruta
if st.button("Buscar ruta desde otro punto"):
    st.session_state.buscar_nueva_ruta = True
    # Reiniciar el estado de la búsqueda
    st.session_state.ruta_optima = None
    st.session_state.distancia_minima = None
    st.session_state.contenedor_cercano = None
    st.session_state.coordenada_origen = None
